...
angular.module('BE.seed.service.inventory', []).factory('inventory_service', [
  ...
  function ($http, $log, urls, user_service, cycle_service, spinner_utility) {
   ...
   /** Get Property View information from server for a specified Property View and Cycle and Organization.
    *
    *  @param property_view_id   	The id of the requested Property View
    *  @param cycle_id            The id of the requested Cycle for the requested Property
    *
    *  @returns {Promise}
    *
    *  The returned Property View object (if the promise resolves successfully) will have a 'state' key with
    *  object containing all key/values for Property State (including 'extra_data')
    *  and a 'cycle' key with an object with at least the "id" key for that Cycle.
    *
    *  An example of structure of the returned JSON is...
    ...
    *    'property_views' {
	*		'certifications': [
	*			... array of certification objects, each with accompanying assessment information
	*		],
    *     'changed_fields': {
    *        'regular_fields' : [
    *          ..list of keys for regular fields that have changed since last state
	...
	**/
    ...
    inventory_service.get_property_view = function (property_view_id, cycle_id) {
      // Error checks
      if (_.isNil(property_view_id)) {
        $log.error('#inventory_service.get_property_view(): property_view_id is undefined');
        throw new Error('Invalid Parameter');
      }
      if (_.isNil(cycle_id)) {
        $log.error('#inventory_service.get_property(): cycle_id is undefined');
      ...
      spinner_utility.show();
//      return $http.get('/api/v2/propertie_views/' + property_view_id + '/', {
	  return $http.get('/api/v2/property_views/538/', {
        params: {
          cycle_id: cycle_id,
          organization_id: user_service.get_organization().id
        }
