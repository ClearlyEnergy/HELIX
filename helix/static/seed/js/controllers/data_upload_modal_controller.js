  ...
  .controller('data_upload_modal_controller', [
    ...
    function ($http,
      ...
      $scope.synchronize_hes = function () {
		  $http.post('/api/v2/hes_upload/', {}, {
          params: {
            organization_id: $scope.organization.org_id,
			  dataset: $scope.dataset.id,
			  cycle: $scope.selectedCycle.id
          }
        }).then(function (data) {
          $scope.dataset.import_file_id = data.data.file;
          $scope.goto_step(3);
        });
      };
      /**
...
// add call to save certifications
			  mapping_service.add_certifications($scope.dataset.import_file_id).then(function (data) {
				  $scope.new_assessments = data.data.new_assessments;
				  $scope.updated_assessments = data.data.updated_assessments;
			  });
